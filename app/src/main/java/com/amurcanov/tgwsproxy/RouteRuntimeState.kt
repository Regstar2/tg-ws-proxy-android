...
data class RouteRuntimeState(
    ...
    val workerFailoverSkippedBackoff: Int = 0,
    ...
    companion object {
        fun fromStatusMap(map: Map<String, String>): RouteRuntimeState {
            ...
            return RouteRuntimeState(
                ...
                selectedWorkerId = map["selected_worker_id"].orEmpty(),
                currentWorkerId = map["runtime_worker_id"].orEmpty().ifBlank { map["current_worker_id"].orEmpty() },
                lastSuccessfulWorkerId = map["last_successful_worker_id"].orEmpty(),
                lastFailedWorkerId = map["last_failed_worker_id"].orEmpty(),
                workerFailoverReason = map["worker_failover_reason"].orEmpty(),
                workerFailoverAttemptCount = map["worker_failover_attempt_count"]?.toIntOrNull() ?: 0,
                workerFailoverActive = map["worker_failover_active"] == "1",
                workerFailoverSkippedBackoff = map["worker_failover_skipped_backoff"]?.toIntOrNull() ?: 0,
                workerSelectionStrategy = map["worker_selection_strategy"].orEmpty(),
                workerSelectionReason = map["worker_selection_reason"].orEmpty(),
                workerCandidateCount = map["worker_candidate_count"]?.toIntOrNull() ?: 0,
